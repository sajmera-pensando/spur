...
# spur_jobs_running > bool 0 converts count to 0/1 — avg = fraction of time jobs were active
...
print(f"  Nodes     {nodes_total} total  |  {nodes_idle} idle  |  {nodes_alloc} alloc  |  {nodes_mixed} mixed  |  {nodes_down} down  |  {nodes_drain} drained")
...
print()
print(f"UTILIZATION -- LAST {HOURS}h")
print(f"  {'Metric':<32}  {'avg':>6}  {'peak':>6}  {'now':>6}  Trend (oldest->now)")
print(f"  {'-'*32}  {'-'*6}  {'-'*6}  {'-'*6}  {'-'*24}")
util_row("Jobs active",                r_jobs)
